...
{
	if (abs(chest_and_key.Top() - character.Top()) <= chest_and_key.Height()) {
		if (abs(chest_and_key.Left() - character.Left()) <= chest_and_key.Width()) {
			chest_and_key.SelectShowBitmap(1);
		}
	}

	if (phase == 5) {
		if (abs(door[0].Top() - character.Top()) <= door[0].Height()) {
			if (abs(door[0].Left() - character.Left()) <= door[0].Width()) {
				door[0].SelectShowBitmap(1);
			}
		}
		if (abs(door[1].Top() - character.Top()) <= door[1].Height()) {
			if (abs(door[1].Left() - character.Left()) <= door[1].Width()) {
				door[1].SelectShowBitmap(1);
			}
		}
		if (abs(door[2].Top() - character.Top()) <= door[2].Height()) {
			if (abs(door[2].Left() - character.Left()) <= door[2].Width()) {
...
void CGameStateRun::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (nChar == VK_LEFT) {
		character.SetTopLeft(character.Left() - 100, character.Top());
	}
	if (nChar == VK_UP) {
		character.SetTopLeft(character.Left() , character.Top() - 100);
	}
	if (nChar == VK_RIGHT) {
		character.SetTopLeft(character.Left() + 100, character.Top());
	}
